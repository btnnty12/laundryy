        ...
        return $table
            ->columns([
                ...
                Tables\Columns\TextColumn::make('pending_orders')
                    ...
                    ->getStateUsing(fn (User $record): int => $record->orders()->where('status', 'pending')->count())
                    ->sortable()
                    ->toggleable(),
                Tables\Columns\TextColumn::make('total_paid')
                    ->label('Total Paid')
                    ->getStateUsing(fn (User $record): string => 'Rp ' . number_format($record->payments()->sum('amount'), 0, ',', '.'))
                    ->sortable()
                    ->toggleable(),
